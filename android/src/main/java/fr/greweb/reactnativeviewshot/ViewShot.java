    ...
    public void execute(NativeViewHierarchyManager nativeViewHierarchyManager) {
        ...
        if (view.getClass().getName().equals("com.oney.WebRTCModule.WebRTCView")) {
            ...
            final Runnable task = new Runnable() {
                @Override
                public void run() {
                    PixelCopy.request(sw, b,new PixelCopy.OnPixelCopyFinishedListener() {
                        @Override
                        public void onPixelCopyFinished(int result) {
                            synchronized (this) {
                                if (result != PixelCopy.SUCCESS) {
                                    Log.d("RNViewShotHack", "Hello world88!!");
                                    promise.reject(ERROR_UNABLE_TO_SNAPSHOT, "Error waiting ");
                                ...
                                try {
                                    ...
                                    fos.flush();
                                    fos.close();
                                    Log.d("RNViewShotHack", "Hello world99!!");
                                } catch (IOException e) {
                                    e.printStackTrace();
                                }
